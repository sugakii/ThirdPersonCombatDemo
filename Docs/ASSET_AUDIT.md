...
- 已生成有效 ImpAvatar；Day 3 用户报告 Idle 重定向视觉正常，助手确认运行中 Idle 状态，未独立目视验收。旧 Rig mismatch 的根因和修复步骤未核实，不虚构结论。
...
- **Loop**：随包说明要求所有以 `_Loop` 结尾的动作在 Unity 中手工开启 Loop Time。当前选用的 Idle/Walk/Jog/Sprint 均已开启 Loop Time；Walk 还开启了 Loop Pose。`A_TPose` 的误配置已经修正并由用户完成 Play Mode 回归。
